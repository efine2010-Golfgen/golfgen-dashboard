...
function ColorCell({ value, onSave }) {
  const [editing, setEditing] = useState(false);
  const [draft, setDraft] = useState(value);
  if (!editing) return (
            ...
            {sorted.map(item => (
              <>
                <tr key={item.asin} style={{ background: saving === item.asin ? "rgba(46,207,170,0.08)" : undefined }}>
                  ...
                  <td><ColorCell value={item.color} onSave={v => handleUpdate(item.asin, "color", v)} /></td>
                  <td style={{ fontSize: 12 }}><EditableCell value={item.brand || ""} type="text" onSave={v => handleUpdate(item.asin, "brand", v)} /></td>
                  ...
                  <td style={{ textAlign: "right" }}><EditableCell value={item.lyRevenue || 0} type="number" prefix="$" onSave={v => handleUpdate(item.asin, "lyRevenue", v)} /></td>
                  <td style={{ textAlign: "right" }}><EditableCell value={item.lyUnits || 0} type="number" onSave={v => handleUpdate(item.asin, "lyUnits", Math.round(v))} /></td>
                  <td>
                    <button
