...
# ═══════════════════════════════════════════════════════════════════════
# TEMPORARY BULK IMPORT — remove after data is loaded
# ═══════════════════════════════════════════════════════════════════════

@router.post("/api/debug/bulk-import")
async def bulk_import(request: Request):
    """Temporary endpoint: accepts JSON rows and inserts into walmart_* tables.
    Body: { "table": "item_weekly|store_weekly|ecomm|forecast", "rows": [[...], ...] }
    """
    body = await request.json()
    table = body.get("table")
    rows = body.get("rows", [])
    if not table or not rows:
        return {"error": "Need table and rows"}

    con = get_db_rw()
    inserted = 0
    try:
        if table == "item_weekly":
            for r in rows:
                try:
                    con.execute("""
    ...
    return {"status": "ok", "table": table, "inserted": inserted, "total_sent": len(rows)}
